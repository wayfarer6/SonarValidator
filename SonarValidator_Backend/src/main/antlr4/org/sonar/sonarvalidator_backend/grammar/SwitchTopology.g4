 ...
 * 어느 쪽으로 토큰화될지 선언 순서에 달렸으므로 둘 다 허용하고,
 * 승격된 설정 키워드도 포함해 `Trunking VLANs Enabled:` 가
 * keyWord+ COLON 으로 매칭되게 한다. */
keyWord     : ATTRWORD | IFNAME | IDENT | VLAN | INTERFACE | STATUSWORD | NUMBER
            | SWITCHPORT | ACCESS | TRUNK | MODEWORD | NATIVE | VLANWORD ;
...
NUMBER     : [0-9]+ ;
/* `Trunking VLANs Enabled: ALL` 의 값. IDENT/ATTRWORD 와 문자집합이 같으므로
 * 먼저 선언해 토큰화되고, visitor 는 타입만 보고 `trunk_vlans_all` 로 분류한다. */
ALL        : 'ALL' ;
...
/* `switchport` 설정 문장의 키워드.
 *
 * `show running-config` 는 `configLine : elem+ NEWLINE` 으로 통째로
 * 위임하지만, 스위치 포트 모드를 판별하는 단어들은 전용 토큰으로 승격한다.
 * 그러면 visitor 가 "이 단어가 switchport 인가" 를 문자열로 묻지 않고
 * 토큰 타입으로 묻는다.
 *
 * ATTRWORD/IFNAME 과 문자집합이 같으므로 먼저 선언해야 동점에서 이긴다. */
